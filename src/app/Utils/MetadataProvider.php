...
class MetadataProvider extends \Prefab
{
	...
	public function __construct()
	{
		...
		$this->limit = isset($GETS['limit']) ? intval($GETS['limit']) : $this->limit;
		$this->offset = isset($GETS['offset']) ? intval($GETS['offset']) : $this->offset;
		if ($this->offset < 0) {
			$this->offset = 0;
		}
		if ($this->limit < 1) {
	...
	public function getDefaultLinks()
	{
		...
		if (
			isset($this->total) && $this->total > 1 && isset($this->count) &&
